    ...
    <div className="flex justify-center">
      <div className="w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl">
        <div className="border border-gray-200 sm:border-2 rounded-lg sm:rounded-xl shadow sm:shadow-lg p-4 sm:p-6 lg:p-8 bg-white">
          <TabGroup>
            <div className="overflow-x-auto py-2 -my-2 -mx-1 px-1">
              <TabList className="flex flex-wrap sm:flex-nowrap justify-center sm:justify-between gap-2 mb-4 sm:mb-6 lg:mb-8 min-w-max">
                {categories.map(({ name }) => (
                  <Tab
                    key={name}
                    className="shrink-0 rounded-md px-3 py-2 sm:px-4 sm:py-2 font-medium text-sm sm:text-base text-gray-700 data-hover:bg-gray-100 data-selected:bg-white data-selected:text-gray-700 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-600 focus-visible:ring-offset-2 min-w-30 sm:min-w-0 relative z-10" // Added relative z-10
                  >
                    {name}
                  </Tab>
                ))}
